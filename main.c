...
int main() {
  ...
  generarEnemigos(enemigos, numEnemigos, &jugador, mapa);
  ...
  // Registro del tiempo del último movimiento de los enemigos
  struct timeval lastEnemyMoveTime, currentTime;
  ...
  while (1) {
    ...
    timeout.tv_sec = 0;
    timeout.tv_usec = 100000; // 0.1 segundos
    ...
    if (rv == -1) {
      ...
      if (input == '6') {
        ...
        usarHabilidad(mapa, &jugador, enemigos, &numEnemigos);
        setNonBlockingMode(1);
      } else {
        moverPersonaje(&jugador, mapa, input);
      }
    }
    if (numEnemigos == 0) {
      numEnemigos = 15;
    ...
    // Verificar si han pasado 2 segundos desde el último movimiento de los
    // enemigos
    if ((currentTime.tv_sec - lastEnemyMoveTime.tv_sec) * 1000 +
            (currentTime.tv_usec - lastEnemyMoveTime.tv_usec) / 1000 >=
        2000) {
      moverEnemigos(enemigos, numEnemigos, &jugador, mapa);
      // Actualizar el tiempo del último movimiento de los enemigos
      gettimeofday(&lastEnemyMoveTime, NULL);
    }
...
void aplicarRegeneracion(Personaje *jugador, int regeneracion) {
  // Esta función debe ser llamada en cada turno del juego para aplicar la
  // regeneración
...
void mostrarHabilidades(Personaje *jugador) { exportarHabilidades(jugador); }
...
Personaje elegirPersonaje() {
  ...
  if (seleccion >= 1 && seleccion <= 4) {
    ...
    FILE *archivo = fopen("Python/personajes.csv",
...
void elegirObjeto(Personaje *jugador) {
  ...
  case 2:
    ...
    int seleccionObjeto2 =
        elegirOpcion(3, 5); // Menu de los objetos específicos
  ...
  case 3:
    ...
    int seleccionObjeto3 =
        elegirOpcion(3, 6); // Menu de los objetos específicos
...
void ganarExperiencia(Personaje *jugador, int exp) {
  ...
  if (jugador->experiencia >= expParaSubir) {
    jugador->nivel++;
    jugador->experiencia -=
        expParaSubir; // Restar la experiencia utilizada para subir de nivel
...
void usarHabilidad(int mapa[MAX][MAX], Personaje *jugador, Enemigo *enemigos, int *numEnemigos) {
  ...
  for (int i = 0; i < *numEnemigos; i++) {
    ...
    if (mapa[enemigos[i].x][enemigos[i].y] != 0 &&
        (enemigos[i].x != jugador->x || enemigos[i].y != jugador->y)) {
      // Calcular distancia entre jugador y enemigo usando la fórmula de
      // distancia euclidiana
      double distancia = sqrt(pow(enemigos[i].x - jugador->x, 2) +
                              pow(enemigos[i].y - jugador->y, 2));
